...
def _run_analysis():
    ...
    currencies = []
    for ccy, cdata in currency_scores.items():
        adj = adjustments.get(ccy, 0)
        currencies.append({
            "currency": ccy, "bias": cdata["bias"], "adj": round(adj, 1),
            "effective": round(cdata["bias"] + adj, 1),
            "mom5": round(cdata.get("momentum_5d", 0), 2),
            "mom20": round(cdata.get("momentum_20d", 0), 2),
        })
